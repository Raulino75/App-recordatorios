...
public class Controller implements ActionListener {
    ...
    private Home home;
    private NewReminder newReminder;

    public void iniciarApp() {
        this.home = new Home();
        this.newReminder = new NewReminder(home, true);
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == home.getBtnAddReminder()) {
            newReminder.setVisible(true);
        }
        if (e.getSource() == newReminder.getBtnAdd()) {
            try {
                addReminder();
            } catch (DuplicateReminderException | InvalidReminderDateException | EmptyInputException | InvalidPriorityException ex) {
    ...
    public void addReminder() throws DuplicateReminderException, InvalidReminderDateException, EmptyInputException, InvalidPriorityException {
        String title = newReminder.getTxtTitle().getText();
        boolean esPremium;
        ...
        String description = newReminder.getTxtaDescription().getText();
        Date date = newReminder.getCalDateCalendar().getDate();
        LocalDate localDate = date.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
        Priority priority = (Priority) newReminder.getCmbPriority().getSelectedItem();
        String location = newReminder.getTxtLocation().getText();
        int index = newReminder.getCmbPlan().getSelectedIndex();
        if (index == 0) {
            esPremium = true;
        } else {
            esPremium = false;
        ...
        reminder.setFecha(localDate.format(DATE_FORMATTER));
        reminder.setPrioridad(priority);
        reminder.setUbicacion(location);

        remindersList.add(reminder);
        JOptionPane.showMessageDialog(null, "¡Recordatorio guardado correctamente!", "Éxito", JOptionPane.INFORMATION_MESSAGE);
    }
    ...
    public LocalDateTime obtainValidDate() throws InvalidReminderDateException, EmptyInputException {
        LocalDateTime dateTime = home.getDate();me
